...
_HIERARCHY_NOTES = os.path.join(_PATH, "hierarchy-notes.txt")
...
# notes: from hierarchy-notes.txt (adding leading "#")
...
def _parse_hierarchy_notes(file_hierarchy_notes, dictionary):
    """Parse the hierarchy-notes.txt file."""
    hierName = ""
    notes = ""
    for line in codecs.open(file_hierarchy_notes, "r"):
        line = line.rstrip()
        if line.startswith(" "):
            ...
            if hierName and hierName in dictionary:
                dictionary[hierName]["notes"] = [notes]
            hierName = line[:-2].upper()
            notes = ""
        elif hierName and hierName in dictionary and not line:
            # All done for the hierarchy!
            dictionary[hierName]["notes"] = [notes]
        else:
            continue
...
if __name__ == "__main__":
    ...
    dictionary = _parse_hierarchy_notes(_HIERARCHY_NOTES, dictionary)
